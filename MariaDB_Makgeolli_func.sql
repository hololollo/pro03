...
-- 자료실 목록(5개씩)
select no, title, content, url, hits, resdate, author from fileboard order by resdate desc limit 0, 5;

-- 자료실 상세보기
select * from fileboard where fno=1;

-- 자료실 글 등록
insert into fileboard values (default, '자료실 글 제목', '자료실 글 내용', '파일명', default, default, 'admin');

-- 자료실 글 수정
update fileboard set title='수정된 자료실 제목', content='수정된 자료실 글' where no=1;

-- 자료실 글 조회수 증가
update fileboard set hits=hits+1 where fno=1;

-- 자료실 글 삭제
delete from fileboard where fno=1;
